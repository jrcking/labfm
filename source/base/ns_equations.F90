...
!! ------------------------------------------------------------------------------------------------
  subroutine update_utran    
     ...
     real(rkind),parameter :: swarm_rate = one!*half*half
     ...
     do i=1,npfb
        ...
        utran(i,1) = utran(i,1) + (minval(h(1:npfb))/dt)*swarm_rate*h0*dYdx!*max(one,max_vel)
        utran(i,2) = utran(i,2) + (minval(h(1:npfb))/dt)*swarm_rate*h0*dYdy!*max(one,max_vel)
        ...
        qkd_mag = (minval(h(1:npfb))/dt)*swarm_rate*(0.2d0+norm_gradY)
        dstmp = zero
        do k=1,ij_count(i)
           j=ij_link(i,k)
           rij=rp(i,:)-rp(j,:);rad=sqrt(dot_product(rij,rij));qq=rad/h(i)
...
!        Zspec(i) = atanh(two*Yspec(i)-one)
        ro(i) = one + two*Ma*Ma*(Yspec(i)-half)*(y-y0)
  ...
  subroutine adjust_for_symmetry_bcs
     ...
     do i=npfb+1,np
        if(rp(i,2).gt.ymax) then
           Yspec(i) = zero
           ro(i) = ro(i) - Ma*Ma*(rp(i,2)-ymax)
        end if
        if(rp(i,2).lt.ymin) then
           Yspec(i) = one
           ro(i) = ro(i) + Ma*Ma*(ymin-rp(i,2))           
        end if
